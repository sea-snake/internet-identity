...
The two boxes group states by whether a value can be served. In the "has a servable value" box, **Refilling** still hands the old value to waiters while it refetches, and a refill failure returns to **Stale** (old value kept); a success lands in **Fresh** (value replaced). **Filling** is the cold path with nothing to serve yet — its failure parks in **CoolingDown**. `evict_at` (hit at `last_success + ttl + stale`, or when over `max_entries`) drops the entry back to **Absent**. Edge labels are the trigger only — per-state behaviour is in the resolution table below.
