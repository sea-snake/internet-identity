...
1. **Failure throttle — exponential backoff debounce.** A failed refresh does not retry on the very next call; it parks `next_attempt = now + delay`, where `delay` grows on consecutive failures and resets on success — the same shape as OpenID's `compute_next_certs_fetch_delay` (§5.5), which this can reuse as a shared helper. Configurable as `RetryBackoff { base_secs, multiplier, max_secs }`: the first failure waits `base_secs` (≪ `ttl`, so a transient blip recovers in about a minute rather than a lifetime), each further consecutive failure multiplies by `multiplier`, capped at `max_secs`. Keep `max_secs ≤ stale` so refreshes keep firing within the servable window and never wait longer than the entry's lifetime. Concrete values are a tuning decision (§10); starting point `base_secs = 60`, `multiplier = 2`, `max_secs = min(stale, 900)`. The first refresh at stale onset (no prior failure) is **not** throttled — `next_attempt` only moves forward after a failure.
...
- **Failure-retry backoff values (§5.2.2).** Shape decided: exponential backoff debounce (`RetryBackoff { base_secs, multiplier, max_secs }`), reset on success, `max_secs ≤ stale`. Open: the concrete values. Starting point `base_secs = 60`, `multiplier = 2`, `max_secs = min(stale, 900)` — first retry a minute after a failure, doubling toward the cap. Tune from the §11 outcall-failure metrics.
